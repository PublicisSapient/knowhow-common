...
public interface SprintRepository extends MongoRepository<SprintDetails, ObjectId> {
	...
	@Query(value = "{ 'basicProjectConfigId' : ?0, 'state' : { $in: [?1] } }", fields = "{'sprintName' : 1, 'startDate' : 1, 'totalIssues' : 1, 'sprintID': 1}", sort = "{ 'startDate' : 1 }")
	...
	@Query(value = "{ 'sprintID': { $in: ?0 } }", fields = "{ 'sprintID': 1, 'startDate': 1, 'endDate': 1, 'sprintName': 1, 'goal': 1 , 'basicProjectConfigId' :  1}", sort = "{ 'startDate': 1 }")
	List<SprintDetails> findBySprintIDInWithFieldsSorted(List<String> sprintIDs);
}
