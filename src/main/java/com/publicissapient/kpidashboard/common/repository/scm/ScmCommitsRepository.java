...
public interface ScmCommitsRepository extends MongoRepository<ScmCommits, String>, ScmCommitRepositoryCustom {
	 ...
	 */
	Page<ScmCommits> findByProcessorItemIdAndCommitTimestampBetween(String processorItemId, LocalDateTime startDate,
	 ...
	 *
	 * @param threshold
	 *          minimum number of total lines affected
	 * @param pageable
	 *          pagination information
	 ...
	 */
	long countByProcessorItemIdAndCommitTimestampBetween(String processorItemId, LocalDateTime startDate,
	 ...
	 */
	@Aggregation(pipeline = {"{'$match': {'processorItemId': ?0}}",
			"{'$group': {" + "'_id': '$commitAuthorId'," + "'commitCount': {'$sum': 1}," + "'totalAddedLines': {'$sum': {'$ifNull': ['$addedLines', 0]}}," + "'totalRemovedLines': {'$sum': {'$ifNull': ['$removedLines', 0]}}," + "'totalChangedLines': {'$sum': {'$ifNull': ['$changedLines', 0]}}," + "'firstCommit': {'$min': '$commitTimestamp'}," + "'lastCommit': {'$max': '$commitTimestamp'}" + "}}",
			"{'$sort': {'commitCount': -1}}"})
	...
	@Aggregation(pipeline = {
			"{'$match': {" + "'processorItemId': ?0," + "'commitTimestamp': {'$gte': ?1, '$lte': ?2}" + "}}",
			"{'$group': {" + "'_id': {'$dateToString': {'format': '%Y-%m-%d', 'date': '$commitTimestamp'}}," + "'commitCount': {'$sum': 1}," + "'totalLinesChanged': {'$sum': {'$add': [{'$ifNull': ['$addedLines', 0]}, {'$ifNull': ['$removedLines', 0]}]}}" + "}}",
			"{'$sort': {'_id': 1}}"})
